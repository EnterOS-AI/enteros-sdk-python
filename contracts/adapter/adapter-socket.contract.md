...
|---|---|---|
| `capabilities() -> RuntimeCapabilities` | all-False `RuntimeCapabilities()` | Declares native ownership of cross-cutting capabilities (heartbeat, scheduler, session, status-mgmt, retry, activity-decoration, channel-dispatch). Platform skips its fallback for a flag the adapter owns. Observability (A2A, activity_logs, broadcaster) is NEVER skipped. claude-code / hermes override. |

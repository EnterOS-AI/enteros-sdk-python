...
that carries **two** IDL conventions. Three language generators cover both;
`gen-runtimes.mjs` additionally projects the shared RuntimeId contract:
...
contracts/<domain>/<name>.contract.json           ─┤  (JSON-Schema draft 2020-12
   ...
   promote-request, workspace-comms                ─┘
        │
        ▼  node tools/gen-runtimes.mjs + tools/gen-<lang>.mjs
...
`contracts/adapter/runtime-id.schema.json` owns the open, bounded, path-safe
RuntimeId shape. `contracts/adapter/official-runtimes.registry.json` separately
owns first-party discovery metadata for Claude Code, Codex, Hermes, and
OpenClaw. It is not an allowlist for third-party adapters.

```sh
node tools/gen-runtimes.mjs
```

The generator inlines the RuntimeId shape into the six marketplace/catalog
schemas, derives known aliases and the official set from the adapter registry,
and emits Python, TypeScript, and Go validators. Known aliases normalize;
unknown valid IDs are preserved.
...
generator. All four generators are Node `.mjs` scripts, so regeneration and the
...
```sh
node tools/gen-runtimes.mjs  # RuntimeId schemas + validators + official discovery
...
| `adapter-registry-conformance` | reconcile the exact official runtime set and adapter-native surfaces with the MCP delivery contract. |
| `prompt-canonical-text-conformance` | verify prompt canonical text and materialization references. |
| `platform-identity-gate-conformance` | run the fail-closed identity gate against its golden cases. |
...
| `bindings-parity` | build/test the TypeScript bindings and build the Python distribution, including RuntimeId exports. |
...
- TS: `@molecule-ai/contracts` (`gen/ts/package.json`), subpaths `./contract`,
  `./workspace-comms`, `./idle-prompt`, and `./runtimes`.
- Python: `molecule-ai-contracts` / import name `molecule_ai_contracts`
  (`gen/python/pyproject.toml`), including top-level RuntimeId helpers and
  official-runtime discovery constants.
